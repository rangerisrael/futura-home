...
import { Bell, X, Trash2, CheckCheck, Plus, Loader2, RefreshCw } from 'lucide-react';
...
const RealNotificationBell = () => {
  ...
  const getNotificationDescription = (notification) => {
    ...
    if (!notification.data) {
      return `A new ${notification.tableName?.toLowerCase() || 'item'} has been added to the system.`;
    }

    const data = notification.data;
    const descriptions = {
      'Property': `New property "${data.property_code || data.unit_number || 'unit'}" has been added to the system.`,
      'Homeowner': `New homeowner "${data.full_name || 'resident'}" has been registered in the system.`,
      'Service Request': `${data.request_type || 'Service'} request: "${data.title || 'New request'}" has been submitted.`,
      'Inquiry': `New inquiry from ${data.full_name || 'visitor'}: "${data.subject || 'General inquiry'}".`,
      'Complaint': `New ${data.complaint_type || 'general'} complaint: "${data.subject || 'Complaint'}" has been filed.`,
      'Announcement': `New announcement: "${data.title || 'Announcement'}" has been published.`,
      'Reservation': `New reservation for ${data.facility_name || 'facility'} on ${data.reservation_date || 'scheduled date'}.`,
      'Billing': `New ${data.bill_type || 'bill'} ${data.amount ? 'for ₱' + Number(data.amount).toLocaleString() : ''} has been generated.`,
      'Transaction': `New ${data.transaction_type || 'transaction'} ${data.amount ? 'of ₱' + Number(data.amount).toLocaleString() : ''} has been processed.`
    };

    return descriptions[notification.tableName] || notification.message || `A new ${notification.tableName?.toLowerCase() || 'item'} has been added to the system.`;
  };
              ...
              >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-600">Connected to Supabase</p>
                      </div>
                      {unreadCount > 0 && (
                ...
                <div className="px-4 py-2 bg-green-50 border-b border-green-200">
                  <p className="text-xs text-green-700 text-center">
                    🟢 Connected to Supabase • Real-time notifications enabled
                  </p>
                </div>
                  ...
                  <div className="max-h-96 overflow-y-auto">
                    {isEmpty(notifications) ? (
                      <div className="px-4 py-8 text-center text-slate-500">
                        ...
                        <p className="text-xs mb-3">Notifications will appear here when new data is added</p>
                        <button
                          onClick={() => handleCreateTestNotification()}
                          disabled={actionLoading?.startsWith('test')}
                          className="text-xs text-blue-600 hover:text-blue-800 underline disabled:opacity-50"
                        >
                          {actionLoading?.startsWith('test') ? 'Creating...' : 'Create test notification'}
                        </button>
                      </div>
                    ) : (
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-1">
                                  <p className="font-medium text-slate-900 text-sm">
                                    {notification.title || `New ${notification.tableName}`}
                                  </p>
                                  {notification.priority && (
                                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                                      ...
                                      notification.priority === 'normal' ? 'bg-blue-100 text-blue-700 border border-blue-200' :
                ...
                {!loading && notifications &&  notifications.length > 0 && (
                  <div className="px-4 py-2 border-t border-slate-200 bg-slate-50 rounded-b-xl">
                    <div className="flex items-center justify-between text-xs text-slate-500">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => handleCreateTestNotification('request_tbl')}
                          disabled={actionLoading?.startsWith('test')}
                        ...
                        <button
                          onClick={() => handleCreateTestNotification('homeowner_tbl')}
                          disabled={actionLoading?.startsWith('test')}
