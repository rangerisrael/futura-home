  ...
  const fetchPaymentDetails = async () => {
    try {
      ...
      if (result.success && result.data.schedule) {
        ...
        setCalculatedPenalty(scheduleData.calculated_penalty || 0);
        setDaysOverdue(scheduleData.days_overdue_after_grace || 0);
        setGracePeriodEnd(scheduleData.grace_period_end);

        // Auto-fill penalty if calculated
        if (scheduleData.calculated_penalty > 0) {
          setPaymentData((prev) => ({
            ...prev,
            penalty_paid: scheduleData.calculated_penalty,
          }));
        }
      }
    } catch (error) {
                  ...
                  <div className="space-y-2">
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <div className="text-xs space-y-1">
                          ...
                          <p className="text-orange-700">
                            Grace Period: 3 days after due date ({gracePeriodEnd})
                          </p>
                          <p className="text-orange-700">
                            Days Overdue: <span className="font-semibold">{daysOverdue} days</span>
                          </p>
                          <p className="text-orange-700">
                            Penalty Rate: 2% per month (0.067% per day)
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="relative">
                      ...
                      <Input
                        ...
                        className="pl-10 bg-orange-50 border-orange-300 font-semibold"
                        readOnly
                      />
                    </div>
                    <p className="text-xs text-orange-600">
                      * Penalty is automatically calculated based on days overdue after grace period
                    </p>
                  </div>
                )}
