...
export async function POST(request) {
  try {
    ...
    const file = formData.get("property_photo");
...
export async function GET() {
  return NextResponse.json(
    { message: "Upload API endpoint. Use POST to upload property images." },
    { status: 200 }
  );
}
...
export async function DELETE(request) {
  try {
    ...
    // Security check - only allow deletion of property images
    if (
      filename &&
      (!filename.startsWith("property-") ||
        filename.includes("..") ||
        filename.includes("/"))
    ) {
    ...
    const storageFilePath = filePath || `properties/${filename}`;
